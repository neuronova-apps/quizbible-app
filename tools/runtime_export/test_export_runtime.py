...
class TestRuntimeExport(unittest.TestCase):
    ...
    def test_export_3john_canonical_to_runtime(self) -> None:
        """Verifica la exportación del banco de 3 Juan al formato runtime."""
        jn3_path = REPO_ROOT / "tools" / "bible_extractor" / "3john-master-input.json"
        if not jn3_path.exists():
            self.skipTest("3john-master-input.json no encontrado")
        raw_jn3 = json.loads(jn3_path.read_text(encoding="utf-8"))
        jn3_qs = raw_jn3.get("questions", raw_jn3)
        status_map = {q["id"]: "VERIFIED" for q in jn3_qs}

        collection = export_canonical_data(jn3_qs, audit_status_map=status_map)
        self.assertEqual(collection["totalQuestions"], 6)
        for q in collection["questions"]:
            self.assertEqual(q["testament"], "NT")
            self.assertEqual(q["book"], "3 Juan")
        self.assertTrue(validate_runtime_collection(collection))
    ...
    def test_export_jude_canonical_to_runtime(self) -> None:
        """Verifica la exportación del banco de Judas al formato runtime."""
        jud_path = REPO_ROOT / "tools" / "bible_extractor" / "jude-master-input.json"
        if not jud_path.exists():
            self.skipTest("jude-master-input.json no encontrado")
        raw_jud = json.loads(jud_path.read_text(encoding="utf-8"))
        jud_qs = raw_jud.get("questions", raw_jud)
        status_map = {q["id"]: "VERIFIED" for q in jud_qs}

        collection = export_canonical_data(jud_qs, audit_status_map=status_map)
        self.assertEqual(collection["totalQuestions"], 12)
        for q in collection["questions"]:
            self.assertEqual(q["testament"], "NT")
            self.assertEqual(q["book"], "Judas")
        self.assertTrue(validate_runtime_collection(collection))

        # Verify TF questions have exactly 2 options A/B
        tf_qs = [q for q in collection["questions"] if q["questionType"] == "TRUE_FALSE"]
        self.assertEqual(len(tf_qs), 2)
        for q in tf_qs:
            self.assertEqual(len(q["options"]), 2)
            self.assertEqual([o["id"] for o in q["options"]], ["A", "B"])
