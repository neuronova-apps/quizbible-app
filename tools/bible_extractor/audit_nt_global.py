#!/usr/bin/env python3
"""
Auditor Global del Nuevo Testamento (27 libros canónicos, 260 capítulos, 1227 preguntas).
...
# ============================================================
# Constantes canónicas del Nuevo Testamento
# ============================================================

EXPECTED_TOTAL_BOOKS = 27
EXPECTED_TOTAL_CHAPTERS = 260
EXPECTED_TOTAL_QUESTIONS = 1227
...
EXPECTED_QUESTION_TYPES: Dict[str, int] = {
    "MULTIPLE_CHOICE": 1072,
    "TRUE_FALSE": 155,
}

EXPECTED_WITH_ADDITIONAL_REFS = 220
EXPECTED_TOTAL_ADDITIONAL_REFS = 281

EXPECTED_MODES: Dict[str, int] = {
    "NT": 1227,
    "AMBOS": 1227,
    "PERSONAJES_NT": 343,
    "PERSONAJES_AMBOS": 343,
...
# Grupos de libros paralelos legítimos del NT (para clasificación semántica)
NT_PARALLEL_GROUPS = [
...
# Orden canónico NT: (order, book_key, canonical_name, filename, total_chapters, total_questions)
CANONICAL_BOOK_ORDER = [
    (1,  "matthew",         "Mateo",            "matthew-master-input.json",         28,  92),
    (2,  "mark",            "Marcos",           "mark-master-input.json",            16,  74),
    (3,  "luke",            "Lucas",            "luke-master-input.json",            24,  96),
    (4,  "john",            "Juan",             "john-master-input.json",            21, 100),
    (5,  "acts",            "Hechos",           "acts-master-input.json",            28, 112),
    (6,  "romans",          "Romanos",          "romans-master-input.json",          16,  80),
    (7,  "1corinthians",    "1 Corintios",      "1corinthians-master-input.json",    16,  80),
    (8,  "2corinthians",    "2 Corintios",      "2corinthians-master-input.json",    13,  65),
    (9,  "galatians",       "Gálatas",          "galatians-master-input.json",        6,  36),
    (10, "ephesians",       "Efesios",          "ephesians-master-input.json",        6,  36),
    (11, "philippians",     "Filipenses",       "philippians-master-input.json",      4,  24),
    (12, "colossians",      "Colosenses",       "colossians-master-input.json",       4,  24),
    (13, "1thessalonians",  "1 Tesalonicenses", "1thessalonians-master-input.json",   5,  30),
    (14, "2thessalonians",  "2 Tesalonicenses", "2thessalonians-master-input.json",   3,  18),
    (15, "1timothy",        "1 Timoteo",        "1timothy-master-input.json",         6,  36),
    (16, "2timothy",        "2 Timoteo",        "2timothy-master-input.json",         4,  24),
    (17, "titus",           "Tito",             "titus-master-input.json",            3,  18),
    (18, "philemon",        "Filemón",          "philemon-master-input.json",         1,   6),
    (19, "hebrews",         "Hebreos",          "hebrews-master-input.json",         13,  78),
    (20, "james",           "Santiago",         "james-master-input.json",            5,  30),
    (21, "1peter",          "1 Pedro",          "1peter-master-input.json",           5,  30),
    (22, "2peter",          "2 Pedro",          "2peter-master-input.json",           3,  18),
    (23, "1john",           "1 Juan",           "1john-master-input.json",            5,  30),
    (24, "2john",           "2 Juan",           "2john-master-input.json",            1,   6),
    (25, "3john",           "3 Juan",           "3john-master-input.json",            1,   6),
    (26, "jude",            "Judas",            "jude-master-input.json",             1,  12),
    (27, "revelation",      "Apocalipsis",      "revelation-master-input.json",      22,  66),
]
...
def classify_semantic_cluster(book_a: str, book_b: str) -> str:
    """
    Clasifica el cluster semántico como paralelo bíblico legítimo, similitud temática
    o mismo libro. NO convierte automáticamente en error a menos que sea un duplicado exacto.
    """
    if book_a == book_b:
        return "MISMO_LIBRO"

    for group in NT_PARALLEL_GROUPS:
...
class GlobalNewTestamentAuditor:
    """
    Auditoría global transversal del Nuevo Testamento.

    Verifica integridad estructural de los 27 bancos, reconcilia los 27 artifacts
    ...
    def __init__(self, base_dir: Optional[Path] = None, output_dir: Optional[Path] = None) -> None:
        ...
        self.output_dir = output_dir or Path("build/audit/nt-global")
        self.manifest_path = self.base_dir / "nt-global-manifest-v1.json"
        self.status_map_path = self.base_dir / "nt-global-audit-status-map-v1.json"
    ...
    def load_manifest(self) -> Dict[str, Any]:
        """Carga y valida el manifiesto oficial del NT."""
        if not self.manifest_path.exists():
            raise FileNotFoundError(f"Manifest NT no encontrado: {self.manifest_path}")
        self.manifest_data = json.loads(self.manifest_path.read_text(encoding="utf-8"))

        if self.manifest_data.get("testament") != "NEW_TESTAMENT":
    ...
    def load_and_validate_all_books(self) -> None:
        """Carga los 27 bancos maestros, verifica SHA-256 contra el manifiesto y estructura."""
        ...
        for order, key, canon_name, filename, exp_ch, exp_q in CANONICAL_BOOK_ORDER:
            ...
            if not m_entry:
                self.issues.append({
                    ...
                    "book_key": key,
                    "detail": "Entrada faltante en manifest NT"
                })
                continue
    ...
    def validate_global_structural_integrity(self) -> Dict[str, Any]:
        """
        Verifica las 1227 preguntas a nivel global: IDs únicos, opciones, dificultad,
        ...
        for q in self.all_questions:
            ...
            if not modes or not isinstance(modes, list):
                ...
                if cat == "PERSONAJES_BIBLICOS":
                    if "PERSONAJES_NT" not in modes:
                        self.issues.append({"type": "PERSONAJES_MISSING_PERSONAJES_NT_MODE", "id": qid})
                    if "PERSONAJES_AMBOS" not in modes:
                        self.issues.append({"type": "PERSONAJES_MISSING_PERSONAJES_AMBOS_MODE", "id": qid})
                for m in modes:
                    if m in ("AT", "PERSONAJES_AT", "VERDADERO_FALSO_AT"):
                        self.issues.append({"type": "UNEXPECTED_AT_MODE_IN_NT", "id": qid, "mode": m})
                    modes_counter[m] += 1
    ...
    def detect_duplicates_and_similarity(self) -> Dict[str, Any]:
        """
        Detecta duplicados exactos, duplicados normalizados y clusters de similitud semántica.
        """
        ...
        for i in range(len(tokenized_questions)):
            ...
            for j in range(i + 1, len(tokenized_questions)):
                ...
                if sim >= 0.75:
                    classification = classify_semantic_cluster(
                        item_a.get("book", ""), item_b.get("book", "")
                    )
    ...
    def load_and_validate_audit_status_map(self) -> Dict[str, Any]:
        """
        Carga el status map NT y valida que contenga exactamente 1227 IDs con estados válidos.
        ...
        if not self.status_map_path.exists():
            raise FileNotFoundError(f"Status map NT no encontrado: {self.status_map_path}")
    ...
    def reconcile_official_audits(self) -> Dict[str, Any]:
        """Reconcilia los estados de las 27 auditorías oficiales desde el manifest."""
        total_verified = 0
    ...
    def validate_global_fail_closed(self, struct_res: Dict, status_map_res: Dict,
        ...
        """
        Valida las condiciones Fail-Closed del global NT.
    ...
    def generate_runtime_and_reports(self) -> Dict[str, Any]:
        ...
        (self.output_dir / "resumen-global-nt.json").write_text(
        ...
        # 2. libros-nt.json
        # -------------------------------------------------------
        (self.output_dir / "libros-nt.json").write_text(
        ...
        sem_summary = {
            ...
            "paralelo_biblico_legitimo": sum(1 for c in self.semantic_clusters if c["classification"] == "PARALELO_BIBLICO_LEGITIMO"),
            "similitud_tematica": sum(1 for c in self.semantic_clusters if c["classification"] == "SIMILITUD_TEMATICA"),
            "mismo_libro": sum(1 for c in self.semantic_clusters if c["classification"] == "MISMO_LIBRO"),
        ...
        md_report = f"""# REPORTE DE AUDITORÍA GLOBAL DEL NUEVO TESTAMENTO (RVR1960)

## 1. Resumen Ejecutivo
- **Libros canónicos**: {len(self.loaded_books)} / 27 (100% cubiertos)
- **Capítulos totales reconciliados**: {audit_res['total_chapters_reconciled']} / 260 (100%)
- **Preguntas totales**: {total_q} / 1227
- **IDs únicos**: {struct_res['unique_ids_count']} / 1227
- **VERIFICADO**: {audit_res['global_verified_count']} ({ver_rate}%)
- **NO_CONCLUYENTE**: {audit_res['global_inconclusive_count']} ({inc_rate}%)
...
- **Categorías**: NT_GENERAL: {struct_res['category_counts'].get('NT_GENERAL', 0)} | PERSONAJES_BIBLICOS: {struct_res['category_counts'].get('PERSONAJES_BIBLICOS', 0)} | JESUS_PALABRAS: {struct_res['category_counts'].get('JESUS_PALABRAS', 0)} | JESUS_MILAGROS: {struct_res['category_counts'].get('JESUS_MILAGROS', 0)} | JESUS_PARABOLAS: {struct_res['category_counts'].get('JESUS_PARABOLAS', 0)}
...
## 3. Modos Elegibles
- NT: {struct_res['eligible_modes_counts'].get('NT', 0)} | AMBOS: {struct_res['eligible_modes_counts'].get('AMBOS', 0)}
- PERSONAJES_NT: {struct_res['eligible_modes_counts'].get('PERSONAJES_NT', 0)} | PERSONAJES_AMBOS: {struct_res['eligible_modes_counts'].get('PERSONAJES_AMBOS', 0)}
...
- **Clusters semánticos detectados**: {dup_res['semantic_clusters_count']} (clasificados por tipo de paralelismo NT)
- **Bancos con SHA-256 intacto**: {len(self.loaded_books)} / 27 (100%)
- **Artefactos oficiales reconciliados**: 27 / 27 (100%)

## 5. Status Map (Fail-Closed)
- **Total entradas**: {status_map_res['total_entries']} / 1227
...
"""
        (self.output_dir / "REPORTE_GLOBAL_NT.md").write_text(md_report, encoding="utf-8")

        return resumen


def main() -> int:
    parser = argparse.ArgumentParser(description="Auditoría Global del Nuevo Testamento")
    parser.add_argument(
        "--base-dir", type=Path, default=Path(__file__).parent,
        help="Directorio base de bancos canónicos NT"
    )
    parser.add_argument(
        "--output-dir", type=Path, default=Path("build/audit/nt-global"),
        help="Directorio de salida para reportes"
    )
    args = parser.parse_args()

    print("Iniciando Auditoría Global del Nuevo Testamento...")
    auditor = GlobalNewTestamentAuditor(base_dir=args.base_dir, output_dir=args.output_dir)
    auditor.load_manifest()
    auditor.load_and_validate_all_books()

    summary = auditor.generate_runtime_and_reports()

    print("Auditoría Global NT completada:")
    print(f"  Libros: {summary['total_books']}/27")
    print(f"  Capítulos: {summary['total_chapters']}/260")
    print(f"  Preguntas: {summary['total_questions']}/1227")
    ...
    if summary["issues_count"] > 0 or summary["audit_summary"]["requires_correction_count"] > 0 or not summary["fail_closed_ok"]:
        print("ERROR: La auditoría global NT detectó incidencias bloqueantes.")
        return 1

    print("AUDITORÍA GLOBAL DEL NUEVO TESTAMENTO: EXITOSA (0 incidencias).")
    return 0
